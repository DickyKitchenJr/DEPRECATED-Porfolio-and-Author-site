...
function nonSenseName() {
    ...
    let digraph = ['wh', 'th', 'sh', 'ph', 'nt', 'ch', 'ck', 'ng', 'wr', 'br', 'gr', 'tr', 'cr', 'kr', 'kn', 'mb', 'gh', 'st', 'sl', 'kl', 'cl', 'bl', 'lc', 'lk'];
    ...
    for (const i in word) {
      ...
      if (consonant.includes(word[each]) && vowel.includes(word[each1]) == false && digraph.includes(checkDigraph) == false && each < word.length -1){
    ...
    for(const i in word){
      ...
      if(vowel.includes(word[each]) && vowel.includes(word[beforeEach]) && vowel.includes(word[afterEach])){
